...
1. **Users & Sessions** - The foundation. User table, session tokens, and validation.
2. **Email & Password** - Signup, login, and password hashing.
3. **Rate Limiting** - Protect your login endpoint from brute force.
4. **Inactivity Timeouts** - Expire sessions after periods of no activity.
5. **OAuth** - Let users sign in with GitHub, Google, etc.
6. **2FA** - Add TOTP-based second factor.
7. **Passkeys** - WebAuthn for passwordless login.

The code uses Go's standard library where possible. For Random IDs, OAuth and WebAuthn, we use [nanoid](https://github.com/matoous/go-nanoid) [Goth](https://github.com/markbates/goth) and [go-webauthn](https://github.com/go-webauthn/webauthn).
